...
int lp_checkbox(lp_ctx *ctx, lp_id id, float x, float y, int *checked, const char *label, int disabled) {
    ...
    lp_draw_outer_shadows(cr, box, LP_RADIUS_XS, hair, 1);
    if (*checked) lp_fill_vgradient(cr, box, accent.light, accent.base, LP_RADIUS_XS);
    else lp_fill_vgradient(cr, box, LP_SURFACE_RAISED_TOP, LP_SURFACE_RAISED_BOTTOM, LP_RADIUS_XS);
    lp_draw_inset_shadows(cr, box, LP_RADIUS_XS, LP_SHADOW_EMBOSS_RAISED, LP_SHADOW_EMBOSS_RAISED_COUNT);
    if (ctx->focus == id && !disabled) lp_draw_focus_ring(cr, box, LP_RADIUS_XS, accent.focus_ring, 3);
