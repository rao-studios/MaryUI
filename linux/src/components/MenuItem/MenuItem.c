...
float lp_menu_item_height(const lp_menu_entry *e) { return e->separator ? 1 + 2 * LP_SPACE_1 : LP_SIZE_CONTROL_HEIGHT; }

lp_size lp_menu_list_measure(cairo_t *cr, const lp_menu_model *m) {
    ...
    for (int i = 0; i < m->count; i++) {
        ...
        h += lp_menu_item_height(e);
        if (e->separator) continue;
        float tw = lp_text_measure(cr, e->label, &ls).w;
        float sw = e->shortcut ? lp_text_measure(cr, e->shortcut, &ss).w + LP_SPACE_2 : 0;
...
void lp_menu_list(lp_ctx *ctx, lp_rect r, const lp_menu_model *m, int active, lp_menu_list_result *out) {
    lp_menu_list_result res = { .selected = -1, .hovered = -1 };
    lp_accent accent = lp_settings_accent(ctx->settings);
    float y = r.y;
    for (int i = 0; i < m->count; i++) {
        const lp_menu_entry *e = &m->entries[i];
        float h = lp_menu_item_height(e);
        lp_rect row = LP_RECT(r.x, y, r.w, h);
        y += h;
        if (e->separator) {
            if (ctx->pass == LP_PASS_DRAW && ctx->cr) {
                lp_fill_solid(ctx->cr, LP_RECT(row.x + LP_SPACE_1, row.y + LP_SPACE_1, row.w - 2 * LP_SPACE_1, 1), LP_EDGE_DIVIDER, 0);
            }
            continue;
        }
        if (lp_hit(ctx, row)) {
...
lp_size lp_menu_popup(lp_ctx *ctx, float x, float y, const lp_menu_model *m, int active, lp_menu_list_result *out) {
    lp_size size = lp_menu_list_measure(ctx->cr, m);
    ...
    lp_menu_list(ctx, LP_RECT(panel.x + LP_SPACE_1, panel.y + LP_SPACE_1, panel.w - 2 * LP_SPACE_1, panel.h - 2 * LP_SPACE_1), m, active, out);
    return size;
}
