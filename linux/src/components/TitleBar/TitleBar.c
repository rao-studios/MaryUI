...
void lp_title_bar(lp_ctx *ctx, lp_rect r, const lp_title_bar_model *m, lp_title_bar_result *out) {
    ...
    float lx = r.x + LP_SPACE_3, ly = r.y + (r.h - lights.h) / 2;
    /* The same inset on both sides, so the title is centred in the bar rather
     * than in the space left over: the lights' full width plus one gutter.
     * (size.titlebar-title-inset is emitted as 90, but the web computes this
     * and does not use it; porting the token would be 8px off.) */
    float inset = lights.w + LP_SPACE_6;
    ...
    if (ctx->pass == LP_PASS_DRAW && ctx->cr) {
        ...
        /* title: absolutely centred, so it does not shift with its own length */
        ...
        lp_text_draw(ctx->cr, m->title ? m->title : "", LP_RECT(r.x + inset, r.y, r.w - 2 * inset, r.h), &st, LP_ALIGN_CENTER);
    }
