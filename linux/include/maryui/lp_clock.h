...
#define LP_CLOCK_INSET 6.0f
#define LP_CLOCK_REST_OPACITY 0.5f    /* at rest: there, not loud */
#define LP_CLOCK_HOVER_OPACITY 1.0f   /* while the pointer is over its text */
#define LP_CLOCK_HIT_PAD 4.0f     /* from the box's right edge, room for the keyline and the shadow */
...
/* The part of `box` that answers the pointer: the time's line, padded by LP_CLOCK_HIT_PAD and kept inside the box.
 * The rest of the chrome falls through to the wallpaper. cr may be NULL (measured on a scratch context). */
