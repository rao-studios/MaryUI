...
The grain moves too, and its offset is a straight proportion of how fast the window is going: `-clamp(v / brush.velocity-ref) * brush.lag`. A slow drag leans it a little, a fling leans it all the way — the hairline scratches visibly slide under the light. There is no spring and no easing here on purpose. A spring sprang the grain back past the frame when a drag stopped, so the metal read as elastic rather than dragged; an easing curve made it react to the gesture instead of tracking it. The only smoothing is a speed limit (`brush.settle`, the longest it may take to cross its full travel), which exists solely so the offset does not jump when a drag ends. `brush.glint` lifts the grain's opacity with `--lp-speed` so the scratches catch the light while moving. Travel is clamped well inside the overhang and the tile is seamless, so no edge ever enters the surface.
