...
| Moving scratches | The grain is an oversized element, not a pseudo-element, so it can be translated: a heavily damped lag spring drags it up to `brush.lag` px behind the frame and springs it back, while `brush.glint` lifts its opacity with speed | `motionEngine.ts`, `Surface.module.css` |
